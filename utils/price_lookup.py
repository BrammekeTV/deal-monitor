...
Cardmarket is scraped via Playwright (no credentials required).

Both lookups are optional; if the required credentials / browser are
absent the lookup is silently skipped and the scorer falls back to the
static market_values defined in config.yaml.
...
# ---------------------------------------------------------------------------
# Cardmarket (Playwright scraper)
# ---------------------------------------------------------------------------

async def _cardmarket_lookup(
    browser: "Browser", query: str
) -> PriceResult | None:
    """Scrape Cardmarket search results for *query* using Playwright."""
    ...
    *browser* is a Playwright Browser instance used for Cardmarket scraping;
    ...
    if settings.cardmarket_enabled and browser is not None:
        ...
        if cm_result:
            ...
            logger.info(
                "Cardmarket: '%s' → avg %.2f EUR (%d listings)",
                query,
                cm_result.avg_price,
